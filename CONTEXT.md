...
| Файл | Назначение |
|---|---|
| `App.tsx` | Главный компонент, навигация (Setup / треки / история / заметки), запуск watcher, listener на `tracks-updated`. Передаёт `setTracks` дочерним компонентам как `onUpdate`. |
| `Setup.tsx` | Первый запуск и настройки: имя + папка группы. Не создаёт структуру автоматически. |
| `TrackCard.tsx` | Карточка трека: статус, варианты с кнопкой "открыть", блок заметок, кнопка "история". Нет ручных кнопок лока. |
| `TrackHistory.tsx` | Страница истории версий трека |
| `TrackNotes.tsx` | Страница заметок трека: список, добавление, редактирование, удаление |
